...
# ==========================================
# DEFINING PATHS FIRST TO PREVENT NAMEERRORS
WATCH_DIR = r'D:\ids\ids_storage'
MODEL_DIR = r'D:\ids\hybrid_DAE-RF'
MASTER_LOG_PATH = r'D:\ids\threat_history_log.csv'

# These are the 45 features coming from your consumer.py
...
# This MUST stay at 43 to match your saved .h5 and .pkl files
...
st.set_page_config(page_title="Shield-IDS Real-Time", layout="wide")

# ==========================================
# 2. MODEL UTILITIES
# ==========================================
def build_dae_architecture(input_dim=43): 
    """Rebuilds the Autoencoder architecture to match saved weights."""
...
@st.cache_resource
def load_assets():
    """Loads all ML models and scalers into memory."""
...
def pre_process_data(df):
    """Aligns incoming 45-feature data with the 43-feature model requirements."""
...
# ==========================================
st.title("🛡️ Shield-IDS: Live Threat Monitor")
st.sidebar.header("System Settings")
auto_refresh = st.sidebar.checkbox("Auto-Scan Storage", value=True)

# Call load_assets (MODEL_DIR is now globally defined above)
lgbm, scaler, dae_full, dae_encoder, rf_model = load_assets()

if 'processed_files' not in st.session_state:
    st.session_state.processed_files = set()

# Scan for CSVs in the storage directory
current_csvs = sorted([f for f in os.listdir(WATCH_DIR) if f.endswith('.csv')])
pending_files = [f for f in current_csvs if f not in st.session_state.processed_files]

if pending_files:
    latest_file = pending_files[0] 
    file_path = os.path.join(WATCH_DIR, latest_file)
    
    try:
        # Give the OS a split second to release any file locks
        time.sleep(0.2)
        df_raw = pd.read_csv(file_path)
        
        if df_raw.empty:
            st.session_state.processed_files.add(latest_file)
            st.rerun()

        # Step 1: Pre-process to 43 features
        X_raw = pre_process_data(df_raw)
        
        # Step 2: DAE & Scaler Transformation
        ...
        # Step 5: Verification (LGBM High Confidence + RF Anomaly)
        ...
        m1.metric("Current File", latest_file)
        m2.metric("Flows Analyzed", len(results_df))
        
        threat_count = int(verified_attacks.sum())
        if threat_count > 0:
            m3.error(f"Threats Detected: {threat_count}")
        else:
            m3.success("Network Clean")

        c1, c2 = st.columns([1, 1])
        with c1:
            st.subheader("Traffic Breakdown")
            fig = px.pie(results_df, names='Status', color='Status', 
                         color_discrete_map={'Benign':'#00CC96', 'ATTACK':'#EF553B'}, hole=0.4)
    ...
    st.info("📡 System standby. Monitoring storage for new network captures...")
    if auto_refresh:
        time.sleep(3)
        st.rerun()